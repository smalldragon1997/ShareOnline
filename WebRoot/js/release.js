...
$(document).ready(function() {
	...
	//提交验证
	$('#releaseSubmitBtn').click(function(){
			...
			$.ajax({
				...
				success : function(data) {
					if (data.state) {
						...
						alert(data.msg);
						setTimeout("$('#submitBtn').removeAttr('disabled').text('确认发布')", 1000); //设置1秒后提交按钮 显示
					}
				},
				error : function() {
					alert('发布失败！服务器崩溃');
	...
	//获得typeList
	$.ajax({
		url : "/shareOnline/comm/getReleaseType",
		dataType : "json",
		type : "POST",
		success : function(data) {
			$.each(data, function(index, comment) {
				//对每一个列表名结果进行遍历
				$('#typeList').append('<option value="'+comment.typeId+'"> '+comment.typeName+' </option>');
			});
		},
		error : function() {
			alert('发布页面 商品类型列表加载失败！服务器崩溃');
		}
	});
